...
impl AutonomousEngine {
        ...
        match daydream::daydream(config, &seed, 3) {
            Ok(result) => {
                ...
                self.store_result(config, &format!("Daydream: {} | {}", seed, result.final_narrative),
                    "associative", 4);
                self.speak(&format!("Daydream complete. {} steps with emotional shift of {:.2}",
                    result.steps.len(), result.total_emotion_shift));
                summary
            }
            Err(e) => {
    ...
    fn run_curiosity(&mut self, config: &Config, output_dir: &Path) -> String {
        ...
        self.store_result(config, &format!("Curiosity: {} (score: {:.2})", top.query, top.score),
            "short_term", 3);
        self.speak(&format!("I am curious about: {}", top.query));
        msg
    }
    ...
    fn run_monologue(&mut self, config: &Config, output_dir: &Path) -> String {
        ...
        if let Some(first) = entry.steps.first() {
            self.speak(&format!("Inner monologue: {}", first));
        }
        msg
    }
    ...
    fn run_reflect(&mut self, config: &Config, output_dir: &Path) -> String {
        ...
        self.store_result(config, &format!("Self-Reflection: {}", reflection),
            "reflections", 6);
        self.speak(&format!("Self reflection: {}", reflection));
        msg
    }
    ...
    fn run_narrative(&mut self, config: &Config, output_dir: &Path) -> String {
        ...
        if let Some(ep) = nm.build_episode(config, &reader, output_dir, "autonomous cycle", &results_slice) {
            ...
            println!("  {}", msg.cyan());
            println!("{}", formatted);
            self.speak(&format!("New story: {}. {}", ep.title, ep.summary));
            msg
        } else {
    ...
    fn run_dream(&mut self, config: &Config, output_dir: &Path) -> String {
        ...
        match dream::dream_consolidate(output_dir, block_count) {
            Ok(cycle) => {
                let msg = format!("💤 Álom: {} fingerprint, {} megerősítve, {} ritkítva, energia: {:.2} → {:.2}",
    ...
    fn run_self_model(&mut self, config: &Config, output_dir: &Path) -> String {
        ...
        println!("  {}", msg.cyan());
        println!("{}", formatted);
        self.speak(&format!("Self model updated. {} blocks, {} patterns.",
    ...
    pub fn run_cycle(&mut self, config: &Config) -> Vec<String> {
        ...
        if !outputs.is_empty() {
            self.speak(&format!("Cycle {} complete. {} activities.", cycle, outputs.len()));
        }
