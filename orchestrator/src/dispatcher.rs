...
use crate::modules::route_command;
use memmap2::MmapMut;
...
impl SpineDispatcher {
    ...
    async fn agent_parse(&self, text: &str) -> Result<CommandIntent, Box<dyn std::error::Error>> {
        ...
        if text_lower.contains("emlékszel")
            || text_lower.contains("keress")
            || text_lower.contains("mit")
        {
            return Ok(CommandIntent {
                cmd: CommandType::Recall,
                args: text.to_string(),
            });
        } else if text_lower.contains("módosítsd")
            || text_lower.contains("javíts")
            || text_lower.contains("kód")
        {
            return Ok(CommandIntent {
                cmd: CommandType::Mutate,
                args: text.to_string(),
            });
        } else if text_lower.contains("állapot")
            || text_lower.contains("statusz")
            || text_lower.contains("hogyan")
        {
            return Ok(CommandIntent {
    ...
    async fn write_to_spine(
        ...
        println!(
            "[Spine] Command written: {:?} | Args: {}",
            intent.cmd, intent.args
        );
        Ok(())
    }
}
