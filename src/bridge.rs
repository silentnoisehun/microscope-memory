...
) -> Result<Json<Vec<FindResponse>>, (StatusCode, String)> {
    let reader = MicroscopeReader::open(&state.config)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let k = params.k.unwrap_or(10);
...
async fn look_memory(
    State(state): State<Arc<AppState>>,
    Query(params): Query<LookQuery>,
) -> Result<Json<Vec<LookResponse>>, (StatusCode, String)> {
    let reader = MicroscopeReader::open(&state.config)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let k = params.k.unwrap_or(10);
    let results = reader.look(&state.config, params.x, params.y, params.z, params.zoom, k);
    let mut response = Vec::new();
...
async fn mql_query(
    State(state): State<Arc<AppState>>,
    Query(params): Query<MqlQuery>,
) -> Result<Json<Vec<MqlResponse>>, (StatusCode, String)> {
    let reader = MicroscopeReader::open(&state.config)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let append_path = std::path::Path::new(&state.config.paths.output_dir).join("append.bin");
    let appended = crate::read_append_log(&append_path);
    let q = crate::query::parse(&params.mql);
    let results = crate::query::execute(&q, &reader, &appended);
    let mut response = Vec::new();
    for r in &results {
        let (text, depth, layer) = if r.is_main {
            let h = reader.header(r.block_idx);
            let t = reader.text(r.block_idx).to_string();
            let l = LAYER_NAMES
                .get(h.layer_id as usize)
                .copied()
                .unwrap_or("unknown")
            ...
            let l = LAYER_NAMES
                ...
                .unwrap_or("unknown")
                .to_string();
            (entry.text.clone(), 4u8, l)
        } else {
            continue;
        };
        response.push(MqlResponse {
            text: crate::safe_truncate(&text, 200),
            depth,
            layer,
            score: r.score,
...
async fn session_log(
    ...
    let n = params.n.unwrap_or(50);
    let file_path = std::path::Path::new(&state.config.paths.layers_dir).join("session.txt");
    if !file_path.exists() {
...
async fn consolidate_sessions(
    ...
    Ok(Json(serde_json::json!({
        "consolidated": summaries.len(),
        "groups": summaries
    })))
}

async fn dream_consolidation(
    State(state): State<Arc<AppState>>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    ...
    match crate::dream::dream_consolidate(output_dir, block_count) {
        Ok(cycle) => Ok(Json(serde_json::json!({
            ...
            "replayed_fingerprints": cycle.replayed_fingerprints,
            "strengthened_pairs": cycle.strengthened_pairs,
            "pruned_pairs": cycle.pruned_pairs,
            "pruned_activations": cycle.pruned_activations,
            "consolidated_patterns": cycle.consolidated_patterns,
            "energy_before": cycle.energy_before,
            "energy_after": cycle.energy_after
        }))),
...
pub async fn run(
    ...
    let v1_routes = Router::new()
        ...
        .route("/find", get(find_memory))
        .route("/look", get(look_memory))
        .route("/mql", get(mql_query))
        .route("/build", post(build_index))
