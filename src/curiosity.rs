...
impl CuriosityQuery {
    ...
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        if data.len() < 20 || &data[0..4] != b"CUR1" { return None; }
        let mut pos = 4;
        let ts = u64::from_le_bytes(data[pos..pos+8].try_into().ok()?); pos += 8;
        let qlen = u16::from_le_bytes(data[pos..pos+2].try_into().unwrap_or([0;2])) as usize; pos += 2;
        let query = if qlen > 0 { String::from_utf8_lossy(&data[pos..pos+qlen]).to_string() } else { String::new() }; pos += qlen;
        let rlen = u16::from_le_bytes(data[pos..pos+2].try_into().unwrap_or([0;2])) as usize; pos += 2;
        let reason = if rlen > 0 { String::from_utf8_lossy(&data[pos..pos+rlen]).to_string() } else { String::new() }; pos += rlen;
        let score = f32::from_le_bytes(data[pos..pos+4].try_into().ok()?);
        Some(Self { timestamp_ms: ts, query, reason, score })
    }
}
...
impl CuriosityState {
        ...
        if let Ok(data) = fs::read(&path) {
            ...
            while pos + 20 <= data.len() {
                if let Some(q) = CuriosityQuery::from_bytes(&data[pos..]) {
                    let size = 4 + 8 + 2 + q.query.len().min(MAX_STR_LEN) + 2 + q.reason.len().min(MAX_STR_LEN) + 4;
                    pos += size;
                    queries.push(q);
                } else { pos += 1; }
            }
    ...
    pub fn generate_queries(&mut self, config: &Config, reader: &MicroscopeReader, output_dir: &Path) -> Vec<CuriosityQuery> {
        ...
        if !due.is_empty() {
            let texts: Vec<String> = due.iter().take(3).map(|&idx| {
            ...
            if !texts.is_empty() {
                new_queries.push(CuriosityQuery {
                    ...
                    reason: format!("{} memories are due for spaced repetition review", due.len()),
        ...
        let mut hot_blocks: Vec<(u32, f32)> = hebb.activations.iter().enumerate()
        ...
        for &(idx, energy) in hot_blocks.iter().take(2) {
            ...
            new_queries.push(CuriosityQuery {
                ...
                reason: format!("block {} has energy {:.2} - highest in the system", idx, energy),
                score: energy * 0.8,
            });
        }
        ...
        if !archetypes.archetypes.is_empty() {
            for arch in archetypes.archetypes.iter().take(2) {
                new_queries.push(CuriosityQuery {
                    timestamp_ms: now_ms,
                    query: format!("What is the '{}' archetype and why is it forming?", arch.label),
                    reason: format!("archetype '{}' has {} members and strength {:.3}", arch.label, arch.members.len(), arch.strength),
        ...
        for p in patterns {
            new_queries.push(CuriosityQuery {
                ...
                reason: format!("thought pattern with {} blocks, frequency {}", p.result_blocks.len(), p.frequency),
        ...
        new_queries.push(CuriosityQuery {
            ...
            reason: format!("{} total blocks, {} hot memories, {} due for review", reader.block_count, hot_blocks.len(), due.len()),
            score: 0.3,
        });
