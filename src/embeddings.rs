...
impl OnnxEmbeddingProvider {
    pub fn new(model_path: &str, tokenizer_path: &str, dim: usize) -> Result<Self, EmbeddingError> {
        let session = Session::builder()
            .map_err(|e| EmbeddingError::ApiError(format!("ONNX session builder: {}", e)))?
            .with_intra_threads(1)
            .map_err(|e| EmbeddingError::ApiError(format!("ONNX threads: {}", e)))?
    ...
    fn embed_inner(&self, text: &str) -> Result<Vec<f32>, EmbeddingError> {
        ...
        // Convert to ort::Value using (shape, vec) to avoid ndarray version issues
        let input_ids_val = Value::from_array(([1, seq_len], ids))
            .map_err(|e| EmbeddingError::ApiError(format!("input_ids val: {}", e)))?;
        let attention_mask_val = Value::from_array(([1, seq_len], attention))
            .map_err(|e| EmbeddingError::ApiError(format!("attention_mask val: {}", e)))?;
        let token_type_ids_val = Value::from_array(([1, seq_len], type_ids))
            .map_err(|e| EmbeddingError::ApiError(format!("token_type_ids val: {}", e)))?;

        let mut session = self.session.lock().map_err(|e| EmbeddingError::ApiError(format!("session lock: {}", e)))?;
        let outputs = session
        ...
        let hidden_dim = *dims.last().unwrap_or(&0) as usize;
        let seq_len_out = if dims.len() == 3 { dims[1] as usize } else { 1 };
