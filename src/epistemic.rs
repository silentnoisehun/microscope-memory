...
pub fn link_evidence(
    ...
    // Determine if this is a truly independent source (C2).
...
/// Compute whether a support link represents a truly new, independent source.
///
/// Uses two checks:
...
fn is_echo_cosine(support_text: &str, claim_text: &str) -> bool {
    ...
    let b = match provider.embed(claim_text) {
        Ok(v) => v,
        Err(_) => return false,
    };
...
mod tests {
    ...
    // ── C2: Cosine echo detection ──────────────────────

    /// C2b: cosine echo — a paraphrase of the claim from the same source
    ...
    fn c2_cosine_echo_same_source_no_increment() {
        ...
        let claim = content_hash("the Microscope is a cognitive memory engine");
        let claim_text = "the Microscope is a cognitive memory engine";
        // Paraphrase of the claim — cosine should be high
