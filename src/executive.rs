...
pub enum ModuleState { Idle, Running, Blocked(String), Error(String), Exhausted }
#[derive(Debug, Clone)]
pub struct CognitiveModule {
    pub id: String, pub name: String, pub priority: u8, pub energy_cost: f64,
    pub state: ModuleState, pub last_run_ms: u64, pub run_count: u64,
    pub avg_duration_ms: f64, pub tags: Vec<String>,
}

#[derive(Debug, Clone)]
pub struct CognitiveResources {
    pub attention_budget: f64, pub energy_level: f64, pub time_budget_ms: u64,
    pub working_memory_load: f64, pub context_switches: u64,
}

#[derive(Debug, Clone)]
pub struct ExecutionPlan {
    pub module_order: Vec<String>, pub total_energy: f64,
    pub estimated_duration_ms: u64, pub priority: u8, pub reason: String,
}

#[derive(Debug, Clone)]
pub struct ExecutionLogEntry {
    pub timestamp_ms: u64, pub module_id: String, pub action: String,
    pub duration_ms: u64, pub energy_used: f64, pub success: bool, pub detail: String,
}

#[derive(Debug, Clone)]
pub struct ExecutiveConfig {
    pub max_modules_per_cycle: usize, pub min_energy_for_execution: f64,
    pub energy_regen_rate: f64, pub critical_energy_threshold: f64,
    pub attention_decay_rate: f64, pub log_retention: usize,
}

impl Default for ExecutiveConfig {
    fn default() -> Self {
        Self {
            max_modules_per_cycle: 5, min_energy_for_execution: 0.1,
            energy_regen_rate: 0.02, critical_energy_threshold: 0.15,
            attention_decay_rate: 0.05, log_retention: 500,
        }
    }
}
...
impl Executive {
    pub fn new() -> Self {
        Self {
            ...
            resources: Arc::new(RwLock::new(CognitiveResources {
                attention_budget: 1.0, energy_level: 1.0, time_budget_ms: 1000,
                working_memory_load: 0.0, context_switches: 0,
            })),
    ...
    pub fn register_module(&self, id: &str, name: &str, priority: u8, energy_cost: f64, tags: Vec<String>) {
        self.modules.write().unwrap().insert(id.to_string(), CognitiveModule {
            id: id.to_string(), name: name.to_string(), priority, energy_cost,
            state: ModuleState::Idle, last_run_ms: 0, run_count: 0, avg_duration_ms: 0.0, tags,
        });
    }

    pub fn module_state(&self, id: &str) -> Option<ModuleState> {
        self.modules.read().unwrap().get(id).map(|m| m.state.clone())
    }

    pub fn set_module_state(&self, id: &str, state: ModuleState) -> bool {
        self.modules.write().unwrap().get_mut(id).map(|m| { m.state = state; true }).unwrap_or(false)
    }
    ...
    pub fn set_priority(&self, id: &str, priority: u8) -> bool {
        self.modules.write().unwrap().get_mut(id).map(|m| { m.priority = priority; true }).unwrap_or(false)
    }

    pub fn resources(&self) -> CognitiveResources { self.resources.read().unwrap().clone() }
    ...
    pub fn schedule(&self) -> ExecutionPlan {
        ...
        let mut sorted: Vec<&CognitiveModule> = modules.values()
            .filter(|m| m.state == ModuleState::Idle || m.state == ModuleState::Running).collect();
        sorted.sort_by(|a, b| b.priority.cmp(&a.priority));
        let order: Vec<String> = sorted.iter().map(|m| m.id.clone()).collect();
        let energy: f64 = sorted.iter().map(|m| m.energy_cost).sum();
        let plan = ExecutionPlan {
            module_order: order.clone(), total_energy: energy,
        ...
        queue.clear();
        for id in &order { queue.push_back(id.clone()); }
    ...
    pub fn execute_next(&self) -> Option<String> {
        ...
        self.log.write().unwrap().push_back(ExecutionLogEntry {
            timestamp_ms: now, module_id: next.clone(), action: "execute".to_string(),
            duration_ms: 100, energy_used: 0.01, success: true,
            detail: "Scheduled by executive".to_string(),
        });
        if let Some(m) = self.modules.write().unwrap().get_mut(&next) {
            m.last_run_ms = now; m.run_count += 1; m.state = ModuleState::Running;
        }
        Some(next)
    }
}
impl Executive {
    pub fn cycle(&self) -> Vec<String> {
        ...
        for _ in 0..config.max_modules_per_cycle {
            if let Some(id) = self.execute_next() { executed.push(id); } else { break; }
        }
        self.update_resources(|r| {
            r.energy_level = (r.energy_level + config.energy_regen_rate).min(1.0);
            if r.energy_level > 0.9 { r.attention_budget = 1.0; }
        });
        executed
    }
    ...
    pub fn stats(&self) -> (usize, usize, f64, f64) {
        ...
        (m.len(), m.values().filter(|m| m.state == ModuleState::Running).count(), r.energy_level, r.attention_budget)
    }

    pub fn recent_log(&self, k: usize) -> Vec<ExecutionLogEntry> {
        self.log.read().unwrap().iter().rev().take(k).cloned().collect()
    }

    pub fn recommend_module(&self, vagus_tone: Option<f64>) -> Option<String> {
        let modules = self.modules.read().unwrap();
        let res = self.resources.read().unwrap();
        let mut candidates: Vec<&CognitiveModule> = modules.values()
            .filter(|m| m.state == ModuleState::Idle && m.energy_cost <= res.energy_level).collect();
        ...
        if let Some(tone) = vagus_tone {
            if tone < 0.4 {
                candidates.sort_by(|a, b| {
                    ...
                    b_stress.cmp(&a_stress).then_with(|| b.priority.cmp(&a.priority))
                });
            } else { candidates.sort_by(|a, b| b.priority.cmp(&a.priority)); }
        } else { candidates.sort_by(|a, b| b.priority.cmp(&a.priority)); }
