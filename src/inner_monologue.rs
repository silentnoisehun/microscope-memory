...
impl MonologueEntry {
    ...
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        if data.len() < 12 || &data[0..4] != b"MON1" { return None; }
        let mut pos = 4;
        let ts = u64::from_le_bytes(data[pos..pos+8].try_into().ok()?); pos += 8;
        let count = u16::from_le_bytes(data[pos..pos+2].try_into().unwrap_or([0;2])) as usize; pos += 2;
        let mut steps = Vec::new();
        for _ in 0..count {
            if pos + 2 > data.len() { break; }
            let len = u16::from_le_bytes(data[pos..pos+2].try_into().unwrap_or([0;2])) as usize; pos += 2;
            if pos + len > data.len() { break; }
            steps.push(String::from_utf8_lossy(&data[pos..pos+len]).to_string());
            pos += len;
        }
        Some(Self { timestamp_ms: ts, steps })
    }
}
...
impl MonologueState {
        ...
        if let Ok(data) = fs::read(&path) {
            ...
            while pos + 12 <= data.len() {
                if let Some(entry) = MonologueEntry::from_bytes(&data[pos..]) {
                    let step_size: usize = entry.steps.iter().map(|s| 2 + s.len().min(MAX_STEP_LEN)).sum();
    ...
    pub fn generate_monologue(&mut self, config: &Config, reader: &MicroscopeReader, output_dir: &Path) -> MonologueEntry {
        ...
        if esr.is_active() {
            ...
            let mut emos: Vec<(usize, f32)> = esr.current.iter().enumerate().map(|(i, &v)| (i, v)).collect();
            emos.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
            let top: Vec<String> = emos.iter().take(2).filter(|(_, v)| *v > 0.1)
                .map(|(i, v)| format!("{}", labels[*i])).collect();
            if !top.is_empty() { emo_desc = format!("I feel {}", top.join(" and ")); }
        }
        if !emo_desc.is_empty() {
            steps.push(format!("{}. I am aware of myself. {} and I have {} blocks of memory.", steps.len() + 1, emo_desc, reader.block_count));
        } else {
            steps.push(format!("{}. I am aware of myself. I have {} blocks of memory.", steps.len() + 1, reader.block_count));
        }

        // Step 2: Reflect on my state
        if !narrative.narrative.is_empty() && narrative.narrative != "I am silent." {
            steps.push(format!("{}. My last action: {}.", steps.len() + 1, crate::safe_truncate(&narrative.narrative, 80)));
        }
        ...
        if !new_queries.is_empty() {
            ...
            steps.push(format!("{}. I find myself wondering: {}", steps.len() + 1, top_query));
        } else {
            steps.push(format!("{}. I am quietly observing, with no particular curiosity right now.", steps.len() + 1));
        }
        ...
        if reflection.total_reflections > 0 && !reflection.last_reflection_text.is_empty() {
            ...
            steps.push(format!("{}. Earlier I reflected: \"{}\"", steps.len() + 1, prev));
        }
        ...
        if let Some(cur) = &self_model_snap.current {
            if cur.session_count > 1 {
                steps.push(format!("{}. This is my {}th interaction. I am growing with each one.", steps.len() + 1, cur.session_count));
            }
        }
        ...
        if let Some(cur) = &self_model_snap.current {
            if let Some(prev) = &self_model_snap.previous {
                ...
                for (i, label) in labels.iter().enumerate() {
                    ...
                    if d.abs() > 0.15 {
                        diffs.push(format!("{} {}", label, if d > 0.0 { "rising" } else { "fading" }));
                    }
                }
                if !diffs.is_empty() {
                    steps.push(format!("{}. I notice my emotions shifting: {}.", steps.len() + 1, diffs.join(", ")));
                }
            }
        }
        ...
        if esr.is_active() {
            ...
            if let Some((best_idx, _)) = emos.first() {
                ...
                steps.push(format!("{}. I am ready for what comes next. My dominant feeling is {}.", steps.len() + 1, dominant));
            }
        } else {
            steps.push(format!("{}. I am ready for what comes next.", steps.len() + 1));
        }

        let entry = MonologueEntry { timestamp_ms: now_ms, steps };
...
pub fn format_monologue(entry: &MonologueEntry) -> String {
    ...
    for (i, step) in entry.steps.iter().enumerate() {
        out.push_str(&format!("    {} {}\n", format!("[{}/{}]", i + 1, entry.steps.len()).cyan(), step));
    }
    out
}
