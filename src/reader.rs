...
impl MicroscopeReader {
    ...
    /// Like find_text but only returns blocks at depth 3-5 (actual content items).
    /// Depth 0-2 are identity/summary/cluster blocks — not useful as recalled memories.
    pub fn find_text_content(&self, query: &str, k: usize) -> Vec<(u8, usize)> {
        let q = query.to_lowercase();
        let mut results: Vec<(u8, usize)> = (0..self.block_count)
            .into_par_iter()
            .filter_map(|i| {
                let h = self.header(i);
                if h.depth >= 3 && h.depth <= 5 && self.text(i).to_lowercase().contains(&q) {
                    Some((h.depth, i))
                } else {
                    None
                }
        ...
        // Prefer depth-3 (full items) over depth-4/5 (sentence fragments)
        results.sort_by_key(|&(d, _)| d);
        results.truncate(k);
        results
    }
