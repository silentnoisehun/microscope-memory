...
impl SynapticPlasticity {
    ...
    pub fn ltp(&mut self, pre_block: u32, post_block: u32) -> f32 {
        ...
        let synapse = self.synapses.entry(key).or_insert_with(|| Synapse {
            pre_block,
            post_block,
            weight: 0.5,
    ...
    pub fn load(dir: &Path) -> Result<Self, String> {
        ...
        if idx + 4 <= data.len() {
            ...
            for _ in 0..synapse_count {
                ...
                idx += 4;
                let last_pre = u64::from_le_bytes([
                    data[idx],
                    data[idx + 1],
                    data[idx + 2],
                    data[idx + 3],
                ...
                idx += 8;
                let last_post = u64::from_le_bytes([
                    data[idx],
                    data[idx + 1],
                    data[idx + 2],
                    data[idx + 3],
    ...
    pub fn heterosynaptic_depression(
        &mut self,
        active_synapse: (u32, u32),
        ...
        if let Some(active) = self.synapses.get(&active_synapse) {
            ...
            let keys_to_weaken: Vec<_> = self
                .synapses
                .keys()
                .filter(|(p_pre, p_post)| {
                    *p_post == post && self.distance(pre, *p_pre) <= neighborhood_radius
                })
                .cloned()
                .collect();
    ...
    pub fn time_dependent_plasticity(
        &mut self,
        synapse_key: (u32, u32),
        practice_count: u32,
        strategy_age_ms: u64,
        ...
        if let Some(synapse) = self.synapses.get_mut(&synapse_key) {
            ...
            let strategy_boost = if strategy_age_ms < 300_000 {
                // 5 minutes
